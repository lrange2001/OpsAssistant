...
- **Esc 不退出模式**:有等待回车的命令时,Esc 先**取消全部等待回车**(撤掉布防,模式保留,照常对话即可);若该会话正在生成,流也一并停。要退出 ops,用 `/mode plan|build|edit|yolo` 显式切换。
...
- 「等待回车」chip 上的 **x** 只取消那一台的等待;**Esc** 取消全部等待回车。取消只是撤掉等待,命令若已在你手里,回不回车仍由你定;模式保留,继续对话即可。终端里正在跑全屏程序(vim/top 等)时,Esc 与回车都是程序按键、原样进终端,不会触发取消或读取。
- 终端断开后,对应等待会被自动清掉(重连会换新终端,旧等待不会触发)。

### 刷新恢复

布防(哪台终端在等回车)按会话持久化:刷新页面或重启应用后,SSH 终端重挂完成时自动还原各会话的等待状态,状态条照常显示;已断开的终端对应等待会被清掉。
